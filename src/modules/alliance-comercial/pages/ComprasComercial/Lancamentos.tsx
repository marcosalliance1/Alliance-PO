...
export default function Lancamentos() {
  const { compras, contasGerenciais, centrosCusto, carregando, erro, criar, remover } = useComprasComercial()

  const [data, setData]                 = useState(hoje())
  const [contaGerencial, setConta]      = useState('')
  const [centroCusto, setCentro]        = useState('')
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault()
    setErroForm(null)
    if (!data || !contaGerencial || !centroCusto || !valor.trim() || !descricao.trim()) {
    ...
    setSalvando(true)
    try {
      await criar({ data, desc_conta_gerencial: contaGerencial, desc_centro_custo: centroCusto, valor: valorNum, descricao: descricao.trim() })
      setData(hoje()); setConta(''); setCentro(''); setValor(''); setDescricao('')
    } catch (err) {
    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="bg-surface rounded-xl border border-white/10 p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className={label}>Centro de Custo <span className="text-primary">*</span></label>
            <select className={base} value={centroCusto} onChange={e => setCentro(e.target.value)}>
              <option value="">Selecione…</option>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  ...
                  <th className="px-4 py-2 text-left text-xs font-semibold text-text-muted whitespace-nowrap">Centro de Custo</th>
              ...
              <tbody>
                {compras.map(c => (
                  <tr key={c.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-2 text-text-main max-w-[220px] truncate" title={c.desc_conta_gerencial}>{c.desc_conta_gerencial}</td>
                    <td className="px-4 py-2 text-text-main max-w-[220px] truncate" title={c.desc_centro_custo}>{c.desc_centro_custo}</td>
                    ...
                    <td className="px-4 py-2 text-text-muted max-w-[260px] truncate" title={c.descricao}>{c.descricao}</td>
