...
\
**\uc0\u10060  What LLMs Do (Wrong Assumptions)**\
\
```python\
...
\
**\uc0\u9989  What Should Happen (Surface Assumptions)**\
\
```\
...
\
**\uc0\u10060  What LLMs Do (Pick Silently)**\
\
```python\
...
\
**\uc0\u10060  What LLMs Do (Overengineered)**\
\
```python\
...
```\
\
**\uc0\u9989  What Should Happen (Simple)**\
\
```python\
...
\
**\uc0\u10060  What LLMs Do (Unnecessary Features)**\
\
```python\
...
```\
\
**\uc0\u9989  What Should Happen (Just What Was Asked)**\
\
```python\
...
- Added docstring\
\
**\uc0\u9989  What Should Happen (Surgical)**\
\
```diff\
  def validate_user(user_data):\
      # Check email format\
...
\
**\uc0\u10060  What LLMs Do (Reformat Everything)**\
\
```diff\
...
\
**\uc0\u9989  What Should Happen (Match Existing Style)**\
\
```diff\
+ import logging\
+ \
...
\
**\uc0\u10060  What LLMs Do (Vague Approach)**\
\
```\
...
```\
\
**\uc0\u9989  What Should Happen (Verifiable Goals)**\
\
```\
...
\
Plan:\
1. Write test: Change password \uc0\u8594  verify old session invalidated\
...
\
**\uc0\u10060  What LLMs Do (All at Once)**\
\
```python\
...
```\
\
**\uc0\u9989  What Should Happen (Incremental with Verification)**\
\
```\
...
1. Add basic in-memory rate limiting (single endpoint)\
   Verify: \
   - Test: 100 requests \uc0\u8594  first 10 succeed, rest get 429\
...
\
**\uc0\u10060  What LLMs Do (Fix Without Reproducing)**\
\
```python\
...
```\
\
**\uc0\u9989  What Should Happen (Reproduce First)**\
\
```python\
...
\
# Verify: Run test 10 times \uc0\u8594  fails with inconsistent ordering\
\
...
| Goal-Driven | "I'll review and improve the code" | "Write test for bug X \uc0\u8594  make it pass \u8594  verify no regressions" |\
\
## Key Insight\
\
The "overcomplicated" examples aren't obviously wrong\'97they follow design patterns and best practices. The problem is **timing**: they add complexity before it's needed, which:\
\
